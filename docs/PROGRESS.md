...
| M1.4 实时文件监控 | ✅ | NTFS USN Journal + Go 独立进程，跨卷监控，动态目录配置，Worker 线程异步处理内容更新，崩溃自动恢复 |
| M1.5 搜索历史 | ✅ | |
| M1.6 保存的搜索 | ✅ | |
...
| 功能 | 状态 | 备注 |
|------|------|------|
| 主线程零阻塞架构 | ✅ | 所有写操作委托给 Worker 线程 |
| 孤立文件自动清理 | ✅ | 扫描前自动清理不在任何扫描目录下的文件 |
| Shard 最大限制调整 | ✅ | 从 2GB 调整为 1GB |
| 数据存储位置自定义 | ✅ | 用户可选择数据存储目录 |
| hot-cache.json 移除 | ✅ | 搜索速度已足够快，不再需要 |

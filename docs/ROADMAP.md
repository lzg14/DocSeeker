...
- [ ] **M1.1** 新增 RTF 文件格式支持
- [ ] **M1.2** 新增 CHM 帮助文档格式支持
- [ ] **M1.3** 新增 ODF 系列格式支持（ODT / ODS / ODP）
- [ ] **M1.4** 重新启用实时文件监控（优化 chokidar 或迁移至 NTFS USN）
- [ ] **M1.5** 搜索历史功能（记录与快速复用）
- [ ] **M1.6** 保存的搜索（收藏夹与命名搜索）
- [ ] **M1.7** 移除 / 大幅提高结果上限（200 条限制）
...
- [ ] **M2.1** 正则表达式搜索支持
- [ ] **M2.2** 布尔搜索支持（AND / OR / NOT）
- [ ] **M2.3** 文件类型过滤器（按扩展名筛选）
- [ ] **M2.4** 文件大小过滤器（范围选择器）
- [ ] **M2.5** 日期范围过滤器（修改时间 / 创建时间）
- [ ] **M2.6** 高级查询语法 UI 提示面板（类 Recoll 查询语言界面）
...
- [ ] **M3.1** ZIP / RAR 压缩包内全文搜索
- [ ] **M3.2** 邮件格式支持（mbox / EML）
- [ ] **M3.3** 便携版打包（索引 + 数据可打包带走）
- [ ] **M3.4** 云存储集成（OneDrive / Dropbox，本地缓存索引）

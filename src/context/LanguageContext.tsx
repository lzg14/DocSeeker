...
const translations: Record<Language, Record<string, string>> = {
  'zh-CN': {
    ...
    'guide.features': '主要功能',
    'guide.feature1': '全文搜索：支持文件名和内容搜索（docx、xlsx、pdf、txt 等格式）',
    'guide.feature2': '定时扫描：可配置定时任务，自动增量更新索引',
    'guide.feature3': '重复文件检测：通过 MD5 哈希快速找出重复文件',
    'guide.feature4': '多文件夹管理：支持同时管理多个扫描目录',
    'guide.advantages': '核心优势',
    'guide.advantage1': '本地优先：所有数据存储在本地，不上传云端，隐私安全',
    'guide.advantage2': '全文检索：支持 docx/xlsx/pdf/txt 等格式的全文内容搜索，不仅仅是文件名',
    'guide.advantage3': '定时扫描：自动增量更新索引，无需手动维护',
    'guide.advantage4': '轻量高效：基于 SQLite + Electron，启动快、资源占用低',
    'guide.tech': '技术栈',
  ...
  'en': {
    ...
    'guide.advantages': 'Core Advantages',
    'guide.advantage1': 'Local-first: All data stored locally, no cloud upload, privacy-safe',
    'guide.advantage2': 'Full-text search: Search content inside docx/xlsx/pdf/txt files, not just filenames',
    'guide.advantage3': 'Scheduled scan: Automatically incremental update index, no manual maintenance',
    'guide.advantage4': 'Lightweight & efficient: Built on SQLite + Electron, fast startup, low resource usage',
